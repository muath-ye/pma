...
/* harmony import */ var _modules_common_ts__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(3);
/* harmony import */ var _modules_tooltip_ts__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(11);
/* harmony import */ var _modules_functions_createProfilingChart_ts__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(74);
/* harmony import */ var _modules_functions_escape_ts__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(17);
/* harmony import */ var _modules_functions_getImageTag_ts__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(18);
/* harmony import */ var _modules_functions_isStorageSupported_ts__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(13);
...
_modules_ajax_ts__WEBPACK_IMPORTED_MODULE_1__.AJAX.registerOnload('server/status/monitor.js', function () {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#logAnalyseDialog').find('.datetimefield').each(function () {
    _modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.addDatepicker(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this));
  });
  ...
  function addChart(chartObj) {
    ...
    var settings = {
      ...
      axes: {
        xaxis: {
          renderer: (jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot).DateAxisRenderer,
          tickOptions: {
            formatString: '%H:%M:%S',
            showGridline: false
          },
    ...
    if (settings.title === window.Messages.strSystemCPUUsage || settings.title === window.Messages.strQueryCacheEfficiency) {
      ...
      settings.axes.yaxis.tickOptions = {
        formatter: jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot.byteFormatter(2) // MiB
      };
    } else if (settings.title === window.Messages.strTraffic) {
      settings.axes.yaxis.tickOptions = {
        formatter: jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot.byteFormatter(1) // KiB
      };
    } else if (settings.title === window.Messages.strQuestions || settings.title === window.Messages.strConnections) {
      settings.axes.yaxis.tickOptions = {
        formatter: function (format, val) {
          if (Math.abs(val) >= 1000000) {
            return jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot.sprintf('%.3g M', val / 1000000);
          } else if (Math.abs(val) >= 1000) {
            return jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot.sprintf('%.3g k', val / 1000);
          } else {
            return jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot.sprintf('%d', val);
          }
        }
      };
    }
    settings.series = chartObj.series;
    if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('#' + 'gridchart' + runtime.chartAI).length === 0) {
      var numCharts = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chartGrid').find('.monitorChart').length;
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chartGrid').find('tr').last().append('<td><div id="gridChartContainer' + runtime.chartAI + '" class="">' + '<div class="ui-state-default monitorChart"' + ' id="gridchart' + runtime.chartAI + '"' + ' style="width:' + chartSize.width + 'px; height:' + chartSize.height + 'px;"></div>' + '</div></td>');
    }
    ...
    chartObj.chart = jquery__WEBPACK_IMPORTED_MODULE_0___default().jqplot('gridchart' + runtime.chartAI, series, settings);
    // remove [0,0] after plotting
    for (i in chartObj.chart.series) {
      chartObj.chart.series[i].data.shift();
    }
    var $legend = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div></div>').css('padding', '0.5em');
    for (i in chartObj.chart.series) {
      $legend.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div></div>').append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>').css({
        width: '1em',
        height: '1em',
        background: chartObj.chart.seriesColors[i]
      }).addClass('float-start')).append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>').text(chartObj.chart.series[i].label).addClass('float-start')).append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div class="clearfloat">')).addClass('float-start'));
    }
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#gridchart' + runtime.chartAI).parent().append($legend);
    if (initialize !== true) {
      runtime.charts['c' + runtime.chartAI] = chartObj;
      buildRequiredDataList();
    }
    // time span selection
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#gridchart' + runtime.chartAI).on('jqplotMouseDown', function (ev, gridpos, datapos) {
      drawTimeSpan = true;
      selectionTimeDiff.push(datapos.xaxis);
      if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('#selection_box').length) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#selection_box').remove();
      }
      var selectionBox = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div id="selection_box" >');
      // eslint-disable-next-line compat/compat
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(document.body).append(selectionBox);
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#gridchart' + runtime.chartAI).on('jqplotMouseUp', function (ev, gridpos, datapos) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#gridchart' + runtime.chartAI).on('jqplotMouseMove', function (ev) {
      if (!drawTimeSpan || editMode) {
        return;
      }
      if (selectionStartX !== undefined) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#selection_box').css({
          width: Math.ceil(ev.pageX - selectionStartX)
        }).fadeIn();
      }
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#gridchart' + runtime.chartAI).on('jqplotMouseLeave', function () {
      drawTimeSpan = false;
    });
    // eslint-disable-next-line compat/compat
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(document.body).on('mouseup', function () {
      if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('#selection_box').length) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#selection_box').remove();
      }
    });
  ...
  function getLogAnalyseDialog(min, max) {
    ...
    _modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.addDatepicker($dateStart, 'datetime', {
      showMillisec: false,
      showMicrosec: false,
      timeFormat: 'HH:mm:ss',
      firstDay: window.firstDayOfCalendar
    });
    _modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.addDatepicker($dateEnd, 'datetime', {
      showMillisec: false,
      showMicrosec: false,
      timeFormat: 'HH:mm:ss',
      firstDay: window.firstDayOfCalendar
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().post('index.php?route=/server/status/monitor/query', {
      ...
      if (data.profiling) {
        ...
        for (i = 0, l = data.profiling.length; i < l; i++) {
          ...
          totalTime += duration;
          numberTable += '<tr><td>' + data.profiling[i].state + ' </td><td> ' + _modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.prettyProfilingNum(duration, 2) + '</td></tr>';
        }
        // Only put those values in the pie which are > 2%
        for (i = 0, l = data.profiling.length; i < l; i++) {
          duration = parseFloat(data.profiling[i].duration);
          if (duration / totalTime > 0.02) {
            chartData.push([_modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.prettyProfilingNum(duration, 2) + ' ' + data.profiling[i].state, duration]);
          } else {
            otherTime += duration;
          }
        }
        if (otherTime > 0) {
          chartData.push([_modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.prettyProfilingNum(otherTime, 2) + ' ' + window.Messages.strOther, otherTime]);
        }
        numberTable += '<tr><td><b>' + window.Messages.strTotalTime + '</b></td><td>' + _modules_functions_ts__WEBPACK_IMPORTED_MODULE_2__.Functions.prettyProfilingNum(totalTime, 2) + '</td></tr>';
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('div.placeHolder a[href="#showNums"]').on('click', function () {
          jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('#queryProfiling').hide();
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('div.placeHolder a[href="#showChart"]').on('click', function () {
          jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('#queryProfiling').show();
          jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('table.queryNums').hide();
          return false;
        });
        profilingChart = (0,_modules_functions_createProfilingChart_ts__WEBPACK_IMPORTED_MODULE_5__["default"])('queryProfiling', chartData);
      }
    });
...
/******/ __webpack_require__.O(0, [41], function() { return __webpack_exec__(73); });
